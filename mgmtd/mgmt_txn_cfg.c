			      ...
			      const char *error_if_any)
{
	...
	if (accept_changes) {
		...
		copy_ret = mgmt_ds_copy_dss(ccreq->dst_ds_ctx, ccreq->src_ds_ctx, create_cmt_info_rec);
	}
	if (discard_changes)
		copy_ret = mgmt_ds_copy_dss(ccreq->src_ds_ctx, ccreq->dst_ds_ctx, false);

	/*
	 * Post-copy admission handoff (B-light).
	 *
	 ...
	 * backend is idle.  Release the DS txn_locks early, clear the active
	 * slot, and move this txn to the finishing slot.  The next CONFIG
	 * create_txn() is no longer blocked on this txn's FE reply or decref.
	 *
	 * We also set ccreq->txn_lock = false so txn_cfg_cleanup() does not
	 * attempt a second unlock on the same DS contexts.
	 *
	...
	/*
	 * Defensive: clear the finishing slot explicitly at the boundary of
	 * the finishing work.  txn_decref() also clears it on refcount=0 but
	 * documenting the handoff here keeps the invariant close to the code
	 * that establishes it.
	 */
